...
DEFINE VARIABLE vXREFFile AS CHARACTER NO-UNDO.
DEFINE VARIABLE vAnalysis AS CHARACTER NO-UNDO.
...
PROCEDURE Main :
    ...
    DO ON ERROR UNDO, RETURN ERROR RETURN-VALUE:
        ...
        IF SUBSTRING(ipOutputDir, LENGTH(ipOutputDir)) NE "/" THEN
        DO:
            ASSIGN ipOutputDir = ipOutputDir + "/".
        END.
    
        ASSIGN vXREFFile = ipOutputDir + ipFileName + ".xref"
               vAnalysis = ipOutputDir + ipFileName + ".analysis".
...
PROCEDURE makeXREF :
    ...
    DO ON ERROR UNDO, RETURN ERROR RETURN-VALUE:
        ...
        MESSAGE "XREF File:" vXREFFile.
    
        COMPILE VALUE(ipFileName) XREF VALUE(vXREFFILE).
    
    END.

END PROCEDURE.

PROCEDURE grepAndReadFile :
    ...
    DO ON ERROR UNDO, RETURN ERROR RETURN-VALUE:
        ...
        INPUT THROUGH VALUE('grep -E " [0-9]* (SEARCH|ACCESS)" ' + vXREFFile).
...
PROCEDURE DumpAnalysis :
    ...
    DO ON ERROR UNDO, RETURN ERROR RETURN-VALUE:
        ...
        MESSAGE "Output File:" vAnalysis.
        
        OUTPUT TO VALUE(vAnalysis) UNBUFFERED.
